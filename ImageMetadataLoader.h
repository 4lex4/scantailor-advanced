...
protected:
     ...
     */
    virtual Status loadMetadata(QIODevice& io_device, VirtualFunction<void, const ImageMetadata&>& out) = 0;

private:
    static Status loadImpl(QIODevice& io_device, VirtualFunction<void, const ImageMetadata&>& out);

    static Status loadImpl(const QString& file_path, VirtualFunction<void, const ImageMetadata&>& out);
...
ImageMetadataLoader::Status ImageMetadataLoader::load(QIODevice& io_device, OutFunc out) {
    ProxyFunction<OutFunc, void, const ImageMetadata&> proxy(out);
