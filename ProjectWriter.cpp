...
void ProjectWriter::enumImagesImpl(VirtualFunction<void, const ImageId&, int>& out) const {
    for (const Image& image : m_images.get<Sequenced>()) {
        out(image.id, image.numericId);
    }
}

void ProjectWriter::enumPagesImpl(VirtualFunction<void, const PageId&, int>& out) const {
