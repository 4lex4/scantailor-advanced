...
QImage dewarpGrayscale(const QImage& src,
    ...
    dst.fill(bg_sample);
    dewarpGeneric<GrayColorMixer<MixingWeight>, uint8_t>(src.bits(), src.size(), src.bytesPerLine(), dst.data(),
                                                         dst_size, dst.stride(), distortion_model, model_domain,
                                                         bg_sample);
...
QImage dewarpRgb(const QImage& src,
    ...
    dewarpGeneric<RgbColorMixer<MixingWeight>, uint32_t>(
            ...
            dst.bytesPerLine() / 4, distortion_model, model_domain, bg_color.rgb());
...
QImage RasterDewarper::dewarp(const QImage& src,
    ...
    if (src.hasAlphaChannel()) {
        return dewarpArgb(src.convertToFormat(QImage::Format_ARGB32), dst_size, distortion_model, model_domain,
                          bg_color);
    } else {
