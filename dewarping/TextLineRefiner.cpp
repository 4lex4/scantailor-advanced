...
private:
    ...
    static float calcBendingEnergy(SnakeNode const& node, SnakeNode const& prev_node, SnakeNode const& prev_prev_node);
...
void TextLineRefiner::refine(std::list<std::vector<QPointF>>& polylines, int const iterations, DebugImages* dbg) const {
    if (polylines.empty()) {
        return;
    }
...
void TextLineRefiner::evolveSnake(Snake& snake, Grid<float> const& gradient, OnConvergence const on_convergence) const {
    float factor = 1.0f;
...
float TextLineRefiner::Optimizer::calcElasticityEnergy(SnakeNode const& node1, SnakeNode const& node2, float avg_dist) {
