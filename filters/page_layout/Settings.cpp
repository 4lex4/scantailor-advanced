...
void Settings::Impl::setPageParams(const PageId& page_id, const Params& params) {
    ...
    const Item new_item(page_id, params.hardMarginsMM(), params.pageRect(), params.contentRect(),
                        params.contentSizeMM(), params.alignment(), params.isAutoMarginsEnabled());
...
Settings::AggregateSizeChanged Settings::Impl::setContentSizeMM(const PageId& page_id, const QSizeF& content_size_mm) {
    ...
    const Container::iterator it(m_items.lower_bound(page_id));
    if ((it == m_items.end()) || (page_id < it->pageId)) {
        const Item item(page_id, m_defaultHardMarginsMM, m_invalidRect, m_invalidRect, content_size_mm,
                        m_defaultAlignment, m_autoMarginsDefault);
        m_items.insert(it, item);
    } else {
...
void Settings::Impl::setPageAutoMarginsEnabled(const PageId& page_id, const bool state) {
    ...
    const Container::iterator it(m_items.lower_bound(page_id));
    if ((it == m_items.end()) || (page_id < it->pageId)) {
        const Item item(page_id, m_defaultHardMarginsMM, m_invalidRect, m_invalidRect, m_invalidSize,
                        m_defaultAlignment, state);
        m_items.insert(it, item);
    } else {
