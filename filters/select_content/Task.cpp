...
FilterResultPtr Task::process(const TaskStatus& status, const FilterData& data) {
    ...
    if (m_ptrNextTask) {
        return m_ptrNextTask->process(status, FilterData(data, data.xform()), ui_data.pageRect(),
                                      ui_data.contentRect());
    } else {
        return FilterResultPtr(new UiUpdater(m_ptrFilter, m_pageId, std::move(m_ptrDbg), data.origImage(), data.xform(),
...
void Task::UiUpdater::updateUI(FilterUiInterface* ui) {
    ...
    QObject::connect(view, SIGNAL(manualPageRectSet(const QRectF&)), opt_widget,
                     SLOT(manualPageRectSet(const QRectF&)));
