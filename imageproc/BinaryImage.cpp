...
void BinaryImage::fill(BWColor const color) {
    if (isNull()) {
...
void BinaryImage::fill(QRect const& rect, BWColor const color) {
    if (rect.isEmpty()) {
        return;
    }
...
void BinaryImage::fillExcept(QRect const& rect, BWColor const color) {
    if (isNull()) {
        throw std::logic_error("Attempt to fill a null BinaryImage!");
    }
...
int BinaryImage::countBlackPixels(QRect const& rect) const {
...
QRect BinaryImage::contentBoundingBox(BWColor const content_color) const {
    if (isNull()) {
        return QRect();
    }
...
QImage BinaryImage::toAlphaMask(QColor const& color) const {
    if (isNull()) {
        return QImage();
    }
...
void BinaryImage::fillRectImpl(uint32_t* const data, QRect const& rect, BWColor const color) {
...
BinaryImage BinaryImage::fromMono(QImage const& image, QRect const& rect) {
...
BinaryImage BinaryImage::fromMonoLSB(QImage const& image, QRect const& rect) {
    return fromMono(image.convertToFormat(QImage::Format_Mono), rect);
}

BinaryImage BinaryImage::fromIndexed8(QImage const& image, QRect const& rect, int const threshold) {
    int const width = rect.width();
    int const height = rect.height();
...
BinaryImage BinaryImage::fromRgb32(QImage const& image, QRect const& rect, int const threshold) {
    int const width = rect.width();
    int const height = rect.height();
...
BinaryImage BinaryImage::fromArgb32Premultiplied(QImage const& image, QRect const& rect, int const threshold) {
    int const width = rect.width();
    int const height = rect.height();
...
BinaryImage BinaryImage::fromRgb16(QImage const& image, QRect const& rect, int const threshold) {
    int const width = rect.width();
    int const height = rect.height();
 ...
 */
bool BinaryImage::isLineMonotone(uint32_t const* const line,
                                 int const last_word_idx,
                                 uint32_t const last_word_mask,
                                 uint32_t const modifier) {
    for (int i = 0; i < last_word_idx; ++i) {
...
int BinaryImage::leftmostBitOffset(uint32_t const* const line, int const offset_limit, uint32_t const modifier) {
...
int BinaryImage::rightmostBitOffset(uint32_t const* const line, int const offset_limit, uint32_t const modifier) {
    int const num_words = (offset_limit + 31) >> 5;
