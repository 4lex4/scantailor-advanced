...
void spreadInDirection(BinaryImage& dst,
    ...
    if (num_steps < COMPOSITE_THRESHOLD) {
        spreadInDirectionLow(dst, dst_cs, dst_relevant_rect, src, src_cs, dx_min, dx_step, dy_min, dy_step, num_steps,
    ...
    spreadInDirection(dst, dst_cs, dst_relevant_rect, tmp, tmp_cs, tmp_images, tmp_cs, tmp_image_size, 0,
                      dx_step * first_phase_steps, 0, dy_step * first_phase_steps, second_phase_steps, rop,
    ...
    if (steps_remaining <= 0) {
        ...
        spreadInDirection(tmp, tmp_cs, tmp.rect(), src, src_cs, tmp_images, tmp_cs, tmp_image_size,
                          dx_min + dx_step * steps_done, dx_step, dy_min + dy_step * steps_done, dy_step,
                          steps_remaining, rop, initial_color, true);
...
void spreadGrayVertical(GrayImage& dst, const GrayImage& src, const int dx, const int dy1, const int dy2) {
    ...
    for (int x = 0; x < dst_width; ++x) {
        for (int dst_segment_first = 0; dst_segment_first < dst_height; dst_segment_first += se_len) {
            ...
            fillExtremumArrayLeftHalf<MinOrMax>(array_center, src_data + x + src_segment_center * src_stride,
                                                src_stride, src_segment_first, src_segment_center);

            fillExtremumArrayRightHalf<MinOrMax>(array_center, src_data + x + src_segment_center * src_stride,
