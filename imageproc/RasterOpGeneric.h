...
namespace imageproc
{
    ...
    template <typename T, typename Op>
    void
    rasterOpGeneric(T* data, int stride, QSize size, Op operation)
    {
        if (size.isEmpty()) {
            return;
        }
    ...
    template <typename T1, typename T2, typename Op>
    void
    rasterOpGeneric(T1* data1, int stride1, QSize size, T2* data2, int stride2, Op operation)
    {
        if (size.isEmpty()) {
            return;
        }
    ...
    template <typename T2, typename Op>
    void
    rasterOpGeneric(BinaryImage const& image1, T2* data2, int stride2, Op operation)
    {
        if (image1.isNull()) {
            return;
        }
    ...
    namespace rop_generic_impl
    {
        ...
        {
        public:
            BitProxy(uint32_t& word, int shift)
                : m_rWord(word),
                  m_shift(shift)
            { }
    ...
    template <typename T2, typename Op>
    void
    rasterOpGeneric(BinaryImage& image1, T2* data2, int stride2, Op operation)
    {
